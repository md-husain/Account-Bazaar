...
//         <div>
//            <h1 className='text-3xl font-bold text-grey-800'>My Listings</h1>
//            <p className='text-grey-600 mt-1'> Manage your social account listings</p>
//         </div>
//         <button onClick={()=>navigate('/crate-listing')} className='bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded font-medium flex items-center space-x-2 mt-4 md:mt-0'>
...
//       {/* Balance Section */}
//       <div className='flex flex-col sm:flex-row justify-between gap-4 xl:gap-20 p-6 mb-10 bg-white rounded-xl border border-grey-200'>
//         {[
...
//         ].map((item,idx)=>(
//           <div className='flex flex-1 items-center justify-between p-4 rounded-lg border border-grey-100 cursor-pointer'>
//             <div className='flex items-center gap-3'>
//               <item.icon className='text-grey-500 w-6 h-6' />
//               <span className='font-medium text-grey-600'>{item.label}</span>
//             </div>
//             <span className='text-xl font-medium text-grey-700'>
//               {currency}
...
//         <div className='bg-white rounded-lg border border-grey-200 p-16 text-center'>
...
//           </div>
//           <h3 className='text-xl font-medium text-grey-800 mb-2'>No listings yet</h3>
//           <p className='text-grey-600 mb-5'>Start by creating your first listing</p>
...
//             <div key={listing.id} className='bg-white rouned-lg border-grey-200 hover:shadow-lg shadow-grey-200/70 transition-shadow'>
//               <div className='p-6'>
...
//                               <div className='bg-white text-grey-600 text-xs rounded border border-grey-200 p-2 px-3'>
...
//                         </div>
//                       </div>
//                       <p className='text-sm text-grey-600'><span>@{listing.username}</span></p>
//                     </div>
//                 </div>
...
//                       <div className='flex items-center space-x-2'>
//                         <TrendingUp className='size-4 text-grey-400'/>
...
//                     <span className='tex-2xl font-bold text-grey-800'>
//                       {currency}
//                       {listing.price.toLocaleString()}
//                       </span>
//                       <div className='flex items-center space-x-2'>
//                         {listing.status !== 'sold' && (
//                           <button onClick={()=>deleteListing(listing.id)} className='p-2 border border-grey-300 rounded-lg hover:bg-grey-50 hover:text-red-500'>
//                             <TrashIcon className='size-4' />
//                           </button>
//                         )}
//                         <button onClick={()=>navigate(`/edit-listing/${listing.id}`)} className='p-2 border border-grey-300 rounded-lg hover:bg-grey-50 hover:text-indigo-600' >
//                           <Edit  className='size-4'/>
//                         </button>

//                         <button onClick={()=>toggleStatus(listing.id)} className='p-2 border border-grey-300 rounded-lg hover:bg-grey-50 hover:text-purple-600'  >
...
import StatCard from '../components/StatCard'
import { platformIcons } from '../assets/assets'
import Credsub from '../components/Credsub'
import WithdrawModel from '../components/WithdrawModel'
import { getAllPublicListing, getAllUserListing, updateListing } from '../app/features/listingSlice'
...
function MyListings() {
  ...
  const handleCredentialSubmit = (listingId, credentials) => {
    dispatch(
      updateListing({
        id: listingId,
        updates: {
  ...
  const toggleStatus = async (listingId) => {
   try {
    toast.loading('Updateing listing status...')
  ...
  const deleteListing = async (listingId) => {
    try {
      const confirm = window.confirm('Are you sure you want to delete this listing? if credentials are changed, new credentiaks will be sent to  your email')
      if(!confirm)return
      ...
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center mb-8 '>
        ...
        <div>
          ...
          <p className='text-gray-600 mt-1'>
            Manage your social account listings
          </p>
        </div>
                    ...
                    <div className='flex justify-between items-start'>
                      ...
                      {listing.credentials?.length ? (
                        <div className='mt-2 text-sm text-gray-600'>
                          ...
                          {listing.credentials.map((c) => `${c.name}: ${c.value}`).join(', ')}
                        </div>
                      ) : null}
