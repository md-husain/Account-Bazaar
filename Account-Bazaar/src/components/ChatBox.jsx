    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-50 flex items-center justify-center sm:p-4">
      ...
      < div className="bg-white sm:rounded-lg shadow-2xl w-full max-w-2xl h-screen sm:h-[600px] flex flex-col">
        ...
        {chat?.listing?.status === "active" ? (
            <form onSubmit={handleSendMessage} className="p-4 bg-white border-grey-200 rounded-b-lg">
                <div className="flex items-end space-x-2"> 
                    ...
                    placeholder="Type your message..." className="flex-1 resize-none border border-grey-300 rounded-lg px-4 py-2 focus:outline-indigo-500 max-h-32" rows={1} />
            ...
            <div classname="p-4 bg-white border-t border-grey-200 rounded-b-lg">
            <p className="text-sm text-grey-600 text-center"> {chat ? `Listing is ${chat?.listing?.status}`:"Loading chat...."}</p>
            </div>
        )}
