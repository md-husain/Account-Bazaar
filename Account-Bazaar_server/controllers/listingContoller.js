...
export const addListing = async (req , res) =>{
    try {
        ...
        accountDetails.username.startsWith("@") ? accountDetails.username = accountDetails.platform.slice(1) : null
...
// controllers for update listing in database
...
export const updateListing = async  (req,res) => {
    try {
        ...
        if(!listing){
            return res.status(404).json({ message: "Listing not found",listing})
        }

        if(listing.status == "sold"){
            return res.status(404).json({ message: " You can't update sold listing",listing})
        }
...
// controller to toggle the status 
...
export const deleteUserListing = async  (req,res) => {
    try {
        ...
        if(!listing){
          ...
          if(listing.status === "sold"){
            return res.status(404).json({ message: "sold listing can't be deleted"})
        }

        // if pswd has been changed , send new password to the owner 
        if(listing.isCredientialChanged){
            // send email to owner
        }
...
export const addCredential = async  (req,res) => {
    try {
        ...
        await prisma.credential.create({
         ...
         await prisma.listing.update({
            where : { id: listingId },
            data: {isCredientialSubmitted: true}
        })
...
// controllers to get all user orders
...
// // controllers to withdrawn amount
...
// // controllers to purchase account
export const purchaseAccount = async  (req,res) => {
    try {
        ...
        if(!listing){
             return res.status(404).json({ message: "Listing not found or not active", withdrawal})

        }
        if(listing.ownerId === userId){
             return res.status(400).json({ message: "You can't purchase your own listing", withdrawal})
